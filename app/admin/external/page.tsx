...
export default function AdminExternalReview() {
  ...
  async function addSourceManual() {
    if (
      ...
      !newSource.feed_url.trim()
    ) {
      alert("Name, base URL, and feed URL are all required.");
      return;
    }
  ...
  async function fetchSuggestions() {
    ...
    if (error) {
      alert(`Suggestion failed: ${error.message}`);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <rect width="100%" height="100%" fill="url(#ext-contour)" />
      </svg>
      <div
            ...
            {(["drafts", "published", "rejected"] as const).map((t) => (
              <button
                ...
                style={{
                  ...
                  boxShadow:
                    tab === t ? `inset 0 0 0 1px ${GOLD_DEEP}` : undefined,
                }}
              >
                {t} ({counts[t]})
              </button>
            ...
            {PILLARS.map((p) => (
              <button
                ...
                style={{
                  ...
                  backgroundColor:
                    pillarFilter === p.value ? "white" : "#f8f4ed",
                }}
              >
                {p.label}
              </button>
